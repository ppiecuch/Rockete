...
QSingleItemSquareLayout::QSingleItemSquareLayout(QWidget* parent, int spacing)
    : QLayout(parent),
    parent(parent)
{
    init(spacing);
...
void QSingleItemSquareLayout::setGeometry(const QRect& rect) {
    ...
    QSize properSize = calculateGoldenRatioSize(rect.size());
    /* Calculate center location in the rect and with item size. */
    QPoint properLocation = calculateTopLeftLocation(rect.size(), properSize);
    /* Set items geometry */
...
QSize QSingleItemSquareLayout::calculateGoldenRatioSize(QSize from) const {
    QSize properSize;
    int w = from.width();
    int h = from.height();
    int calc_h = h ;
    ...
    properSize.setHeight(calc_h - this->margin());
    properSize.setWidth(calc_w - this->margin());
    return properSize;
}
...
/* Calculate the top left location from the given height and width for item size. */
QPoint QSingleItemSquareLayout::calculateTopLeftLocation(QSize from,
                                                        QSize itemSize) const
{
    QPoint topLeft;
    topLeft.setX(0);
